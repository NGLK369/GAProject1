...
let player1Hand = [];
let player2Hand = [];
let gameStatus = ''; // Added game status variable
...
const dealerStandBtn = document.querySelector('#dealerStandBtn'); // Added dealer Stand button
...
const gameStatusElement = document.querySelector('#gameStatus'); // Added game status element
...
function enablePlayerButtons() {
  ...
  dealerStandBtn.disabled = false; // Enable dealer Stand button
}
...
function checkStatus() {
  ...
  if (isBust(player1Hand)) {
    gameStatus = 'Player 1 Busts! Player 2 and Dealer Win!';
  } else if (isBust(player2Hand)) {
    gameStatus = 'Player 2 Busts! Player 1 and Dealer Win!';
  } else if (isBust(dealerHand)) {
    gameStatus = 'Dealer Busts! Player 1 and Player 2 Win!';
  ...
  } else if (dealerTotal > player1Total && dealerTotal > player2Total && dealerTotal <= 21) {
...
function dealerStand() {
  ...
  hitBtn.disabled = true; // Disable the dealer's Hit button
  checkStatus();
}
...
function nextGame() {
  ...
  updateUI();
  displayTotal();
  gameStatusElement.textContent = ''; // Clear game status
}
...
hitBtn.removeEventListener('click', player1Hit); // Remove player 1's hit functionality from the dealer's hit button
hitBtn.addEventListener('click', dealerHit); // Add dealer's hit functionality
